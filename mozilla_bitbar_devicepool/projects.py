...
def upload_project_application_file(id, filepath):
    """Upload application file located at filepath to project.

    :param id: integer id for project.
    :param filepath: path to file.

    Examples:
        upload_project_application_file(250331, /path/ignore.apk
    """
    response = TESTDROID.upload(path='projects/{}/files/application'.format(id), filename=filepath)
    return {'id':response['id'], 'name': response['name'], 'size': response['size']}


def upload_project_data_file(id, filepath):
    """Upload data file located at filepath to project.

    :param id: integer id for project.
    :param filepath: path to file.

    Examples:
        upload_project_data_file(250331, /path/datafile
    """
    response = TESTDROID.upload(path='projects/{}/files/data'.format(id), filename=filepath)
    return {'id':response['id'], 'name': response['name'], 'size': response['size']}


def upload_project_test_file(id, filepath):
    """Upload test file located at filepath to project.

    :param id: integer id for project.
    :param filepath: path to file.

    Examples:
        upload_project_test_file(250331, /path/test.zip
    """
    response = TESTDROID.upload(path='projects/{}/files/test'.format(id), filename=filepath)
    return {'id':response['id'], 'name': response['name'], 'size': response['size']}
